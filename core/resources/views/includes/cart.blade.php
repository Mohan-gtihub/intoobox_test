 ...
 <div class="table-responsive">
    <table class="table">
      ...
      <tbody id="cart_view_load" data-target="{{ route('cart.get.load') }}">
        ...
        @php
            $pro=\App\Models\Item::whereSlug($item['slug'])->first();
        ...
        <tr>
          ...
          <td class="align-middle">
            ...
            <div class="qtySelector product-quantity counter-container d-flex align-items-center bg-gray-100 px-5 py-3 w-fit">
              ...
              data-target="{{ PriceHelper::GetItemId($key) }}"  data-multiple="{{ $pro->multiple }}">
              ...
              data-item="{{ implode(',', $item['options_id']) }}"  data-multiple="{{ $pro->multiple }}">
          ...
          <td class="text-danger fs-18 align-middle">
            <a class="remove-from-cart  border-0" href="{{ route('front.cart.destroy', $key) }}" data-toggle="tooltip" title="Remove item"><i class="fas fa-times text-danger fs-18"></i></a>
          </td>
        </tr>
        @endforeach
      </tbody>
  ...
  <div class="d-flex flex-column flex-lg-row align-items-lg-center justify-content-lg-between mt-6">
    <div>
      <a href="{{ route('front.catalog') }}" class="text-uppercase w-100 w-lg-auto px-6 py-3 btn btn-danger text-white rounded-0  border-0"> Continue ShopPing</a>
    </div>
    <div class="mt-4 mt-lg-0">
      <a href="{{ route('front.cart.clear') }}" class="text-uppercase w-100 w-lg-auto px-6 py-3 btn btn-danger text-white rounded-0  border-0">Clear Cart</a>
    </div>
  </div>
  <!-- cart action buttons -->
  ...
  <div class="row justify-content-lg-between mt-8">
      ...
      <form method="post" id="coupon_form" action="{{ route('front.promo.submit') }}" class="mt-6">
        ...
        <div class="row align-items-center">
          ...
          <div class="col-md-4 mt-2 mt-lg-0">
            <button type="submit" class="btn btn-secondary btn-block w-100 w-lg-auto rounded-0 border-1 border-secondary pt-3 pb-2 text-white text-uppercase  border-0">
              APPLY COUPON
            </button>
          </div>
        </div>
    ...
    <div class="col-lg-4">
      <div class="mt-4 mt-lg-0 p-7 border border-2 w-100 w-lg-auto">
        ...
        <div class="mt-6">
          <a data-toggle="tooltip" title="Remove item" href="{{ route('front.promo.destroy') }}" class="btn btn-danger remove-from-cart text-white btn-block w-100 rounded-0  border-0 text-uppercase  {{ Session::has('coupon') ? '' : 'd-none' }}">
            REMOVE COUPON
          </a>
          <a href="{{ route('front.checkout.billing') }}" class="btn btn-danger text-white btn-block w-100 rounded-0 text-uppercase  border-0">
            PROCEED TO CHECKOUT
          </a>
        </div>
      </div>
